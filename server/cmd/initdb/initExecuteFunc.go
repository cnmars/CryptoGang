...
func (h MysqlInitHandler) WriteConfig(ctx context.Context) error {
	...
	global.GD_CONFIG.System.DbType = "mysql"
	global.GD_CONFIG.Mysql = c
	global.GD_CONFIG.JWT.SigningKey = uuid.Must(uuid.NewV4()).String()
	cs := utils.StructToMap(global.GD_CONFIG)
	for k, v := range cs {
		global.GD_VP.Set(k, v)
	}
	return global.GD_VP.WriteConfig()
}

// EnsureDB 创建数据库并初始化 mysql
func (h MysqlInitHandler) EnsureDB(ctx context.Context, conf *models.InitDB) (next context.Context, err error) {
