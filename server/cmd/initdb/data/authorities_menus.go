...
func (i *initMenuAuthority) InitializeData(ctx context.Context) (next context.Context, err error) {
	...
	// 666
	menu333 := menus[7:10]
	menu333 = append(menu333, menus[12])
	if err = db.Model(&authorities[1]).Association("SysBaseMenus").Replace(menu333); err != nil {
		return next, err
	}
...
func (i *initMenuAuthority) DataInserted(ctx context.Context) bool {
	...
	if ret := db.Model(auth).
		Where("authority_id = ?", 333).Preload("SysBaseMenus").Find(auth); ret != nil {
		if ret.Error != nil {
			return false
		}
