...
import (
	...
	common "golddigger/common/models/response"
...
func (s *SystemApi) GetSystemConfig(c *gin.Context) {
	...
	common.OkWithDetailed(response.SysConfigResponse{Config: config}, "获取成功", c)
}
...
func (s *SystemApi) SetSystemConfig(c *gin.Context) {
	...
	if err != nil {
		...
		common.FailWithMessage("设置失败", c)
		return
	}
	common.OkWithMessage("设置成功", c)
}
...
func (s *SystemApi) ReloadSystem(c *gin.Context) {
	err := utils.Reload()
	if err != nil {
		global.GD_LOG.Error("重启系统失败!", zap.Error(err))
		common.FailWithMessage("重启系统失败", c)
		return
	}
	common.OkWithMessage("重启系统成功", c)
}
...
func (s *SystemApi) FetchEthPriceOnece(c *gin.Context) {
	...
	common.OkWithDetailed(gin.H{"coinPrice": data}, "获取成功", c)
}
func (s *SystemApi) GetUpgradePrice(c *gin.Context) {
	data, err := systemConfigService.GetUpgradePrice()
	if err != nil {
		global.GD_LOG.Error("获取套餐价格失败!", zap.Error(err))
		common.FailWithMessage("获取失败", c)
		return
	}
	common.OkWithDetailed(gin.H{"upgradePrice": data}, "获取成功", c)
}
